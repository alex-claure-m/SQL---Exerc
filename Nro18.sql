...
SELECT r.rubr_detalle as [RUBRO]
	...
	, ISNULL((SELECT TOP 1 p1.prod_codigo FROM Producto p1 
		...
		ORDER BY SUM(ifac1.item_cantidad) DESC),0) AS [PRODUCTO 1] -- por la cantidad mas alta
	, ISNULL((SELECT TOP 1 p1.prod_codigo FROM Producto p1 
		JOIN Item_Factura ifac1 ON ifac1.item_producto = p1.prod_codigo
		JOIN Rubro r1 ON r1.rubr_id = p1.prod_rubro -- matcheame el rubro con el pruducto del rubro
		...
		GROUP BY p1.prod_codigo
		ORDER BY SUM(ifac1.item_cantidad) DESC),0) AS [PRODUCTO 2] -- por la cantidad mas alta
	-- FALTA CLIENTE
FROM Rubro r
